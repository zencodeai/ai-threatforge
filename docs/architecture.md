...
| Attribute | Detail |
|---|---|
| **Source** | `src/analysis/threat_generation.py`, `src/analysis/threat_outputs.py`, `src/analysis/materializer_registry.py`, `src/analysis/heuristics/` (auto-discovered: `th_001`–`th_006` as Python modules, `rules/th_001.toml`–`th_044.toml` for config-driven rules), `src/analysis/heuristics/generic_materializer.py`, `src/analysis/mapping_catalog.py`, `src/analysis/mapping_engine.py`, `src/analysis/mapping_loader.py`, `src/analysis/mapping_types.py`, `src/analysis/threat_enricher.py`, `src/analysis/graphrag_scorer.py` |
...
**MITRE ATT&CK / ATLAS mapping** is handled by a layered mapping engine split across four modules: `mapping_types.py` (data model), `mapping_loader.py` (TOML I/O and knowledge-base name resolution), `mapping_catalog.py` (curated catalog and coverage helpers), and `mapping_engine.py` (curated + tactic-expansion + context-filtering pipeline). Each `rule_id` maps to one or more `TechniqueMapping` entries containing the technique ID (e.g., `T1190`, `AML.T0016`), tactic category (e.g., Initial Access, Defense Evasion), and a rationale explaining the alignment. Coverage validation ensures every heuristic has at least one technique mapping.
